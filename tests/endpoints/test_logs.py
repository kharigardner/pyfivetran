...
class TestLogEndpoint:
    ...
    def test_create_service(self, log_endpoint, mocker):
        ...
        response = log_endpoint.create_service(
            group_id="123", service="Test Service", enabled=True, key="value"
        )
    ...
    def test_delete_service(self, log_endpoint, mocker):
        ...
        assert response == {"status": "success"}
        log_endpoint._request.assert_called_once_with(
            method="DELETE", url=f"{BASE_API_URL}/{API_VERSION}/external-logging/456"
        )
    ...
    def test_get_service(self, log_endpoint, mocker):
        ...
        assert response == {"status": "success"}
        log_endpoint._request.assert_called_once_with(
            method="GET", url=f"{BASE_API_URL}/{API_VERSION}/external-logging/789"
        )
    ...
    def test_test_service(self, log_endpoint, mocker):
        ...
        assert response == {"status": "success"}
        log_endpoint._request.assert_called_once_with(
            method="POST", url=f"{BASE_API_URL}/{API_VERSION}/external-logging/123/test"
        )
    ...
    def test_update_service(self, log_endpoint, mocker):
        ...
        log_endpoint._request.assert_called_once_with(
            ...
            json={"enabled": True, "config": {"key": "value"}},
        )
