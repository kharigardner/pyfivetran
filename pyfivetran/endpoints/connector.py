...
class Connector(ApiDataclass):
    ...
    def delete(self) -> GeneralApiResponse:
        ...
        try:
            obj = self.endpoint.client.delete(
                url=self.as_url
            )
    ...
    def resync(self, scope: Optional[Dict[str, List[str]]] = None) -> GeneralApiResponse:
        """
    ...
    def _from_dict(cls, endpoint, d: Dict[str, Any]) -> 'Connector':
        ...
        if d.get('succeeded_at') and isinstance(d.get('succeeded_at'), str):
            d['succeeded_at'] = datetime.strptime(d.get('succeeded_at'), '%Y-%m-%dT%H:%M:%SZ')  # type: ignore
        if d.get('created_at') and isinstance(d.get('created_at'), str):
            d['created_at'] = datetime.strptime(d.get('created_at'), '%Y-%m-%dT%H:%M:%SZ')  # type: ignore
...
class ConnectorEndpoint(Endpoint):
    ...
    def get_config_metadata(
        ...
        return self._request(
            method='GET',
            url=f'{self.BASE_URL}/metadata/connector-types/{service}'
        ).json()
    ...
    def get_connector_state(
        ...
        return self._request(
            method='GET',
            url=f'{self.BASE_URL}/connectors/{connector_id}/state'
        ).json()
